package com.javathreads.chapter4;
/*
 *
...
public class MultiHostSocketFactory {
	...
	class ConnectThread extends Thread {
		...
		ConnectThread(String host, int port, MultiHostSocketFactory parent) {
			this.host = host;
			this.port = port;
			this.parent = parent;
		}
	...
	public Socket makeSocket(String hosts[], int port) {
		...
		synchronized (this) {
			for (i = 0; i < hosts.length && sock == null; i++) {
				ConnectThread th = new ConnectThread(hosts[i], port, this);
				target = th;
				th.start();
				try {
					wait(30 * 1000);
	...
	public static void main(String args[]) {
		System.out.println("Got " + new MultiHostSocketFactory().makeSocket(args, 7));
	}
}
